...
int fun(int n, vector<int> &v,vector<vector<int>> &dp) {
    if (chk(n, v))  return 1;

    f(i, v.size()) {
        if (n - v[i] >= 11 && fun(n - v[i], v)) {
            dp[n][i] =  1;
        }
    }
    return 0;
    ...
    void solve(){
        ...
        maken
            vi v = {11,111,1111,11111,111111,1111111,11111111};
        vector<vector<int>> dp(n+1,vector<int>(n,v.size()));n
        if(fun(n,v,dp)) c("YES")
        else c("NO")
