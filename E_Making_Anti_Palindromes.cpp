    ...
    void solve(){
        ...
        if(n%2==1){ c(-1);return;}
            ...
            if(same.size()%2==0){
                for(int i=0;i<same.size()/2;i++){
                    if(same[i]==same[same.size()-i-1]){
                        ...
                        notsame.pb({same[i] , same[same.size()-i-1]});
                        notsame.pb({same[same.size()-i-1] , same[i]});
                        c++;
                    } 
                }
            }
            else if(same.size()%2==1){
                for(int i=0;i<same.size()/2;i++){
                    if(same[i]==same[same.size()-i-1]){
                        ns.pb(same[i]);
                        ns.pb(same[i]);
            ...
            if(ns.size()==0){
                ...
                f(i,notsame.size()){
                    if((notsame[i].first != ns[0] ) && (notsame[i].second != ns[0])){
                       __cnt++;
                    }
                  }
            // c("_cnt "<<__cnt)
