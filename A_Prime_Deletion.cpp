                        ...
                        void printSubsequences(vi &arr, int index,
                                               vector<int> &subarr, int n,int &mx,vi &v)
                        {
                            // Print the subsequence when reach
                            // the leaf of recursion tree
                            if (index == n)
                            {    int lcm_ = findlcm(subarr);
                                 if (binary_search(v.begin(),v.end(), lcm_)==0){
                                     int sz = subarr.size();
                                     mx = max(sz, mx);
                                 }
                                //  c("BS "<<search(v, lcm_))
                            ...
                            else
                            {
                                ...
                                printSubsequences(arr, index + 1, subarr, n,mx,v);
