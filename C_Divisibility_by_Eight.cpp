...
#include <cmath>
    ...
    void calcSubset(vector<char>& A, vector<vector<char> >& res,
...
void solve() {
    string s = "1234"; // Sample input string, replace with your actual string
    int n = s.length();
    vector<char> ss(n);
    for (int i = 0; i < n; i++) {
        ss[i] = s[i];
    ...
    for (int i = 0; i < res.size(); i++) {
        ...
        // Check if temp is a valid integer before conversion
        bool isValid = true;
        for (char c : temp) {
            if (!isdigit(c)) {
                isValid = false;
