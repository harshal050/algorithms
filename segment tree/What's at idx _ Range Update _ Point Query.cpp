                                ...
                                void update(int node , int s,int e , int l,int r,int &val,vi &arr){
                                    ...
                                    if(s>=l && e<=r){
                                        ...
                                        update(node*2+1 , s , m , l, r,val,arr);
                                        update(node*2+2 , m+1 , e , l, r,val,arr);
                                        // lazy[node] = lazy[node*2+1]  + lazy[node*2+2] ;   
                                    }
                                   }


                                int query(int node , int s,int e , int ind , vi &arr){
                                    ...
                                    if(s==ind && e==ind){
                                        ...
                                        int m = (s+e)/2;
                                        if(ind>=0 && ind<=m){
                                            return query(node*2+1 , s , m , ind,arr);
                                        }else if(ind>=m+1 && ind<=e){
                                            return query(node*2+2 , m+1 , e , ind,arr);
                                        }
                                    }
                                }


                                void build_fun(int node , int s,int e ,  vi &arr){
                                    if(s==e){
                                        lazy[node] = arr[s];
                                    }
                                    else
                                   ...
                                   void solve(){
                                        ...
                                        while(x--){
                                            ...
                                            if(chk){
                                                mkabc
                                                update(0,0,n-1,a-1,b-1,c,v); 
                                            }else{
                                                int ind;
                                                cin>>ind;

                                                int ans = query(0,0,n-1,ind-1,v);
                                                c(ans)
                                            }
                                            f(i,7) cout<<lazy[i]<<" ";
                                            cout<<endl;
