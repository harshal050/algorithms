                                    ...
                                    void propagate(int node){
                                        int t = a[node*3+2];
                                        a[node*3+2] = a[node*3+1];
                                        a[node*3+1] = a[node*3+0];
                                        a[node*3+0] = t;
                                    ...
                                    void build(int node , int s,int e, vi &v){
                                        if(s==e){
                                            if(v[s]%3==0){
                                                a[node*3+0] = 1;
                                            }
                                            ...
                                            int m = (s+e)/2;
                                            build(node*2+1 , s,m,v);
                                            build(node*2+2 , m+1,e,v);
                                            
                                            
                                            lazy[node] = 0;
                                            if(node<3){

                                            a[node*3+0] = a[(node*2+1)*3+0] + a[(node*2+2)*3+0];
                                            a[node*3+1] = a[(node*2+1)*3+1] + a[(node*2+2)*3+1];
                                            a[node*3+2] = a[(node*2+1)*3+2] + a[(node*2+2)*3+2];
                                            
                                            }
                                        }
                                    }

                                    void update(int node , int s, int e  , int l ,int h){
                                        if(s>h || e<l) return; 
                                        if(s==e){
                                            lazy[node]++;
                                            return;
                                            ...
                                            int m = (s+e)/2;
                                            update(node*2+1 , s ,m,l,h);
                                            update(node*2+2 , m+1 ,e,l,h);
                                        }
                                    }

                                    // query(0,0,n-1,a-1,b-1);

                                    int query(int node , int s, int e , int l ,int h){
                                        // if(s>h || e<l) return 0; 

                                        if(s==e){
                                            if(lazy[node]==1) propagate2(node);
                                            return (a[node*3]);
                                        }
                                        else{
                                            int m = (s+e)/2;
                                            ...
                                            if(s>=l && e<=h){
                                                ...
                                                if(lazy[node]==1) propagate(node);
                                                
                                                a = query(node*2+1 , s,m,l,h);
                                                b = query(node*2+2 , m+1,e,l,h);
                                            }
                                            return a+b;
                                        }
