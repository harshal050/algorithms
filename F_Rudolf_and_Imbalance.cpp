...
int Binary_search(vi x, int size, int t1, int t2, int ele, int c)
{
    int maximum = size - 1;
    int minimum = 0;
    int mean;
    ...
    while (maximum > minimum)
    {
        ...
        if (x[mean] + ele > t1 && x[mean] + ele < t2)
        {
            if (abs(c - (x[mean] + ele)) < mn)
            {
                mn = x[mean] + ele;
            }
            if (x[mean] + ele > c)
                maximum = (mean - 1);
            else if (x[mean] + ele < c)
                minimum = (mean + 1);
            else
                return c;
        }
        else if (x[mean] + ele > t2)
        {
            maximum = (mean - 1);
        }
        else if (x[mean] + ele < t1)
        {
            minimum = (mean + 1);
        }
...
void solve()
{
    ...
    f(i, a)
    {
        mxx = Binary_search(e, b, l, h, d[i], c_val);
        maxi = (mxx, maxi);
    }
