...
#define debug(x, y, z, w) cout << #x << "=" << x << "," << #y << "=" << y << "," << #z << "=" << z << "," << #w << "=" << w << endl
...
int fun(int x, vi h, int a, int r, int m)
{
    int sm = 0;
    int n = h.size();
    int cost = 0;
    ...
    else
    {
        ...
        {
            int cur_val = abs(h[cur] - x);
            // c("cur_val " << cur_val);
            if (h[cur] < x)
            {   
                for (int j = j_ind; j >= cur + 1; j--)
                {
                    j_ind = j;
                    if (h[j] >= x)
                    {
                        ...
                        if (h[j] - x >= cur_val)
                        {   
                            // c("fir cur_val "<<cur_val)
                            h[j] -= cur_val;
                            h[cur] += cur_val;
                            cost += (cur_val * m);
                            f = 1;
                            // c("sec cur_val "<<cur_val)
                            break;
                        }
                        else
                        {
...
int ts(vi &h, int a, int r, int m)
{
    srt(h);
    int left = h[0];
    int right = h[h.size() - 1];
    // pri(h);
    ...
    while(left<right){
        ...
        int fm1 = fun(m1,h,a,r,m);
        int fm2 = fun(m2,h,a,r,m);
        if(fm1<fm2){
            right = m2-1;
        }else if(fm1>fm2){
            left = m1+1;
