        ...
        vi divisors(int n) { vi v; for(int i = 2; i*i <= n; i++) { if(n % i == 0) { v.pb(i); if(i != n / i) { v.pb(n / i); } } } return v; }
        ...
        bool ff(string &s , umapc &mp){
            ...
            for(auto it:mp){
                if((it.ss==1 && mp.size()==2 ) || (it.ss =  s.size() && mp.size()==1)) return 1;
            }    
            return 0;
        }
            ...
            f(i,n){
                if(v[i]<0 && i<n-1){
                    if(v[i+1]<0){
                        v[i]-=2*v[i];
                        v[i+1]-=2*v[i+1];
                    }else if((v[i]-2*v[i])>(v[i+1]-2*v[i+1]) && v[i]+v[i+1] < (v[i]-2*v[i])+(v[i+1]-2*v[i+1])){
                        v[i] = v[i]-2*v[i];
                        v[i+1] = v[i+1]-2*v[i+1];
                    }else i++;
                }
                    ...
                    if(v[i]<0 && v[i]-2*v[i] > v[i-1]-2*v[i-1] && (v[i]+v[i-1] < (v[i]-2*v[i])+(v[i-1]-2*v[i-1]))){
                        v[i]-=2*v[i];
                        v[i-1]-=2*v[i-1];
                    }
                }
