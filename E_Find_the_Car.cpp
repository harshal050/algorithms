    ...
    void solve()
    {
        ...
        while (c--)
        {
            ...
            int ind = lower_bound(v.begin(), v.end(), x) -  v.begin() - 1;
             
            // c("ind "<<ind)
            // if(ind==0) ans = 1ll*(x - v[ind])*(p[ind + 1] - p[ind]) / (v[ind + 1] - v[ind]);
            ans = v[ind] + 1ll*(x - v[ind])*(p[ind + 1] - p[ind]) /  (v[ind + 1] - v[ind]);
